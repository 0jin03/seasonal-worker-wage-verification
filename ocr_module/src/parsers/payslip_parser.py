...
def _payment_and_deductions(page, result, debug, warnings):
    ...
    if not table:
        return
    lines = table["ocr_lines"]
    pay_label_header = next(
        (line for line in lines if key(line["raw_text"]) == key("임금항목")), None
    )
    deduction_label_header = next(
        (line for line in lines if key(line["raw_text"]) == key("공제항목")), None
    )
    amount_headers = [
        line for line in lines
        if key(line["raw_text"]) in {key("지급금액(원)"), key("공제금액(원)")}
    ]
    if pay_label_header is None or deduction_label_header is None or len(amount_headers) < 2:
        return

    # Some official payslip forms print "지급금액(원)" above both the pay and
    # deduction amount columns.  PaddleOCR reads that text correctly, so the
    # two amount columns must be distinguished by their horizontal positions.
    amount_headers.sort(key=lambda line: _center(line)[0])
    pay_amount_header = min(
        (line for line in amount_headers if _center(line)[0] > _center(pay_label_header)[0]),
        key=lambda line: _center(line)[0],
        default=None,
    )
    deduction_amount_header = max(amount_headers, key=lambda line: _center(line)[0])
    if pay_amount_header is None or deduction_amount_header is pay_amount_header:
        return

    headers = {
        "임금항목": pay_label_header,
        "지급금액(원)": pay_amount_header,
        "공제항목": deduction_label_header,
        "공제금액(원)": deduction_amount_header,
    }
    header_y = sum(_center(line)[1] for line in headers.values()) / len(headers)
