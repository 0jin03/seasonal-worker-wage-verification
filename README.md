...
- 합성 PDF 기준 OCR 성능과 실제 촬영문서의 성능에는 차이가 있을 수 있음
- 은행 거래행 중복·누락, 일부 근무기록 행·비고 누락 가능성
- Checkbox 및 날짜·공백·표 Anchor 인식 정확도 개선 필요
- 초기 OCR 대상 은행 양식은 NH농협은행 형식을 중심으로 검증함
- 사업장 업종, 별도 합의, 지역별 행정지침 등 입력에 없는 사실은 판단할 수 없음
- 규칙 결과는 문서 정합성 및 추가 확인 상태를 나타내며 최종 법률 판단을 대신하지 않음
- 캄보디아어 표현은 실제 원어민 사용자를 대상으로 자연스러움과 이해도 추가 검증이 필요함
- 기관 도입을 위한 계정·권한, 이력 관리, 개인정보 마스킹 기능은 시제품 범위에 포함되지 않음

### 향후 개선 방향

1. **실제 촬영문서 평가**  
   기울기·흐림·저해상도 등 실제 촬영 조건별 OCR 성능 및 수정률 측정

2. **OCR Parser 고도화**  
   은행 거래 deduplication, table/overall OCR fallback, Visual ROI 기반 Checkbox 판정, 안전한 normalization 확대

3. **사용자 검증**  
   외국인 계절근로자와 기관 담당자를 대상으로 설명 이해도·사용성 평가

4. **다국어 확장**  
   캄보디아어 원어민 검수 후 베트남어 등 지원 언어 확대

5. **기관형 기능 추가**  
   계정·권한, 검수 이력, 개인정보 마스킹, 법령 갱신 및 사례관리 기능 구현

6. **적용 범위 확대**  
   다른 지역·은행·업종·문서 양식으로 Parser와 Rule Mapping 확장

7. **인접 검증 영역 확장**  
   4대보험·퇴직금·임금체불 상담의 사전 자료 확인 등으로 확대
