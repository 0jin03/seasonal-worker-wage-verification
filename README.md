...
| 브랜치 | 내용 |
|---|---|
| `main` | 파이프라인 코드, 스키마, 피처정의서, 케이스 생성기 |
| `data` | 합성 케이스 데이터셋 전체 및 문서 샘플 |

데이터는 용량이 커 `data` 브랜치로 분리했습니다. 필요하면:
...
`data` — `main`에서 분기했으므로 위 코드·스펙을 그대로 포함하며, 추가로 데이터셋이 있습니다:
...
│   임금명세서.pdf / 입금내역서.pdf
└── 계절근로자_결함케이스_30세트_기타/_백업/     # 케이스 개정 이력
```
...
두 경로 모두 `data` 브랜치의 데이터를 가리키므로, 데이터로 돌려보려면 먼저 `git switch data`가 필요합니다. 결과는 `실행결과/`에 생성되며 버전 관리에서 제외됩니다.
